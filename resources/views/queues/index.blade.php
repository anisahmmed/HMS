    ...
    <div class="p-6">
        ...
        <nav>
            <ul class="space-y-2">
                ...
                <li>
                    <a href="{{ route('queues.index') }}" class="flex items-center p-3 rounded group transition-colors duration-300 bg-white bg-opacity-30">
                        <svg class="w-7 h-7 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-white overflow-hidden shadow-lg sm:rounded-lg">
            <div class="p-6">
                @if(Auth::user()->hasRole('Front Desk/Billing Staff'))
                    <a href="{{ route('queues.create') }}" class="bg-gradient-to-r from-blue-500 to-purple-500 text-white font-bold py-2 px-4 rounded mb-4 inline-block">
                        Issue New <span :class="sidebarOpen ? 'inline' : 'hidden group-hover:inline'" class=ml-3>Token</span>
                    </a>
                @endif
                ...
                @if(Auth::user()->hasRole('Doctor'))
                    <form method="POST" action="{{ route('queues.call-next') }}" class="mb-4 inline-block">
                        ...
                        <button type="submit" class="bg-gradient-to-r from-green-500 to-teal-500 text-white font-bold py-2 px-4 rounded">
                            Call Next <span :class="sidebarOpen ? 'inline' : 'hidden group-hover:inline'" class=ml-3>Patient</span>
                        </button>
                    </form>
                @endif
                ...
                <table class="min-w-full bg-white rounded-lg overflow-hidden shadow-sm">
                    <thead class="bg-gradient-to-r from-blue-500 to-purple-500 text-white">
                        <tr>
                            <th class="py-3 px-4 text-left">Token Number</th>
                            <th class="py-3 px-4 text-left">Patient</th>
                            <th class="py-3 px-4 text-left">Doctor</th>
                            <th class="py-3 px-4 text-left">Status</th>
                            <th class="py-3 px-4 text-left">Issued Time</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="py-3 px-4 border-b font-bold">{{ $queue->token_number }}</td>
                            <td class="py-3 px-4 border-b">{{ $queue->patient->user->name }}</td>
                            <td class="py-3 px-4 border-b">{{ $queue->doctor->user->name }}</td>
                            ...
                            <td class="py-3 px-4 border-b">{{ $queue->issued_time->format('H:i') }}</td>
                            <td class="py-3 px-4 border-b">
                                @if(Auth::user()->hasRole('Doctor') && $queue->status == 'called')
                                    <form method="POST" action="{{ route('queues.serve', $queue) }}" class="inline">
                                        @csrf
                                        @method('POST')
                                        <button type="submit" class="text-green-500">Serve</button>
                                    </form>
                                    |
                                ...
                                @else
                                    <a href="{{ route('queues.show', $queue) }}" class="text-blue-500">View</a>
                                @endif
                            </td>
                        </tr>
                        @endforeach
